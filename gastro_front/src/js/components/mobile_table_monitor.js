...
import { Table, Input, Popconfirm, Divider, Row, Col, Steps, Icon, Pagination} from 'antd';
...
组件:用于实时监控的表 Mobile
*/
export default class MobileTableMonitor extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      data ,
  ...
  render() {
    ...
    this.columns = [{
        ...
        render: (text, record) =>{
          return (
            <div>
              <div id="m_table_card_title_container">
                <span>{record.serial}</span>
                <Divider type="vertical" />
                <span>{record.gastro_name}</span>
                <Divider type="vertical" />
                <span>{record.gastro_id}</span>
                <Divider type="vertical" />
                <span>{record.wash_staff}</span>&nbsp;于&nbsp;<span>{record.wash_station}</span>&nbsp;正在操作
              </div>
    ...
    return(
      <div className="basic_table_and_form_container" style={{paddingTop:"12px"}}>
        <Table dataSource={this.state.data} columns={this.columns} scroll={{x:'1000px'}}
        showHeader={false} pagination={{ size:"small",pageSize:7}}/>
      </div>
    );
  }
}
